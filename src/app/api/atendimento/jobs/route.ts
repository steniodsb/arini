...
async function executar() {
  ...
  const [despertadas, sla, campanhas, encerradas] = await Promise.all([
    despertarAdiadas(admin),
    marcarViolacoesSla(admin),
    processarCampanhas(admin),
    encerrarInativas(admin),
  ]);
  return NextResponse.json({ ok: true, despertadas, sla, campanhas, encerradas });
}
