...
import { NewContractDialog } from "../contratos/NewContractDialog";
...
/** Aba de Contratos dentro do Jurídico (vínculo com imóveis). */
export async function ContratosSection({ canManage }: { canManage: boolean }) {
      ...
      <div className="flex items-center justify-between">
        <div className="grid grid-cols-3 gap-4 flex-1 mr-4">
          <Card><CardContent className="pt-4 pb-4">
            ...
            <div className="text-xl text-arini font-semibold">{(contracts ?? []).length}</div>
          </CardContent></Card>
          <Card><CardContent className="pt-4 pb-4">
            <div className="text-xs uppercase text-muted-foreground">Assinados</div>
            <div className="text-xl text-emerald-600 font-semibold">{(contracts ?? []).filter((c) => c.status_assinatura === "assinado").length}</div>
          </CardContent></Card>
          <Card><CardContent className="pt-4 pb-4">
            <div className="text-xs uppercase text-muted-foreground">Pendentes</div>
            <div className="text-xl text-amber-600 font-semibold">{(contracts ?? []).filter((c) => ["pendente","digital","fisica"].includes(c.status_assinatura)).length}</div>
          </CardContent></Card>
        </div>
