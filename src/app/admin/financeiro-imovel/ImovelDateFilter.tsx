...
export function ImovelDateFilter({ from, to }: { from: string; to: string }) {
  ...
  function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (f) qs.set("from", f);
    if (t) qs.set("to", t);
    router.push(`/admin/financeiro-imovel${qs.toString() ? `?${qs}` : ""}`);
  }
  ...
  return (
    <form onSubmit={submit} className="flex flex-wrap items-end gap-2 pt-2">
      ...
      <Button type="submit" size="sm" variant="outline">Filtrar período</Button>
    </form>
  );
}
